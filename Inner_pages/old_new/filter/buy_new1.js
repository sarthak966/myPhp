...
let newProduct = JSON.parse(sessionStorage.getItem("Products"));
...
let cat=localStorage.getItem("cata");
let catu=localStorage.getItem("cata").toUpperCase();


if(cat==="Mobiles or Smartphones" || cat==="Gadgets and Accessories" || cat==="Dresses or Clothings" || cat==="Women Clothings"){
    for (let i = 0; i < newProduct.length; i++) {
        ...
        if (tag) {
            console.log(tag.toUpperCase())
            if (tag.toUpperCase().indexOf(catu) > -1) {
                items[i].style.display = ""
...
//set search option ac to condition
...
else if(cat==="Refublished"){
    for (let i = 0; i < newProduct.length; i++) {
        let items = document.getElementsByClassName("box");
        let condition = newProduct[i][4];
        let itemsearch = document.getElementsByClassName("i_list");
        console.log(itemsearch[i])
...
//set search bar ac to condition
